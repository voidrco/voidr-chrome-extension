...
class DefectsService {
  ...
  async makeAPIRequest(endpoint, method = 'GET', data = null) {
    return new Promise((resolve) => {
      try {
        console.log('[DefectsService.api] →', method, endpoint, data ? JSON.stringify(data).slice(0, 500) : '');
      } catch (_) {}
  ...
  sanitizeCreateData(data) {
    const navInfo = {
      ...
      browser: (typeof navigator !== 'undefined' && (navigator.userAgent || 'Unknown')) || 'Unknown'
    };
    return {
  ...
  async updateDefect(idOrSlug, updates) {
    const res = await this.makeAPIRequest(`/defects/${encodeURIComponent(idOrSlug)}`, 'PATCH', updates);
  ...
  async uploadAttachment(file, metadata = {}) {
    ...
    const storageResponse = await window.privateStorageService.uploadFile(file, 'defects', metadata);
